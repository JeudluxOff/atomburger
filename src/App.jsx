...
function Recruitment({ restaurants, onSubmitApplication }) {
  ...
  const submit = async event => {
    ...
    try {
      ...
      setForm({ fullName: '', phone: '', role: rankOptions[0], restaurant: selectedRestaurant, message: '' })
      setSent(true)
    } catch (err) {
    ...
    <EmployeePage title="Administration" subtitle="Gestion du service et des contenus.">
      ...
      {tab === 'menu' && <section className="menu-config-layout">
        ...
        <form className="admin-form menu-config-form" onSubmit={addMenu}>
          ...
          <div className="form-row"><input required min="0" step="0.1" type="number" placeholder="Prix" value={draftMenu.price} onChange={e => setDraftMenu({ ...draftMenu, price: e.target.value })} /><input min="0" step="1" type="number" placeholder="Stock" value={draftMenu.stock} onChange={e => setDraftMenu({ ...draftMenu, stock: e.target.value })} /><select value={draftMenu.badge} onChange={e => setDraftMenu({ ...draftMenu, badge: e.target.value })}>{badgeOptions.map(item => <option key={item} value={item}>{item || 'Aucun badge'}</option>)}</select></div>
